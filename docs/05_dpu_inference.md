...
- Loading the compiled `.xmodel` into DPU fabric registers
- Allocating DMA buffers for input/output tensors
- Submitting inference jobs and waiting for completion
...
```diff
- img_int8 = rgb.astype(np.uint8)           # WRONG: dtype is uint8
- input_tensor_data[0] = img_int8            # xir sees uint8, expects int8 → CRASH
```
